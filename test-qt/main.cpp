...
int main(int argc, char *argv[])
{
    ...
    MainWindow w;
	//Thread * thread = new Thread(0, &w);
	//thread->moveToThread(thread);
	//w.mapper->moveToThread(thread);
	//w.mapper->moveToThread(thread);
	...
	//QObject::connect(w.mapper,SIGNAL(mapped(QString)), thread, SLOT(start()), Qt::QueuedConnection);
	//QObject::connect(w.mapper,SIGNAL(mapped(QString)),thread,SLOT(run(QString)), Qt::QueuedConnection);
